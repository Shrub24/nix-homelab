...
- [x] 1.2 Add `treefmt.toml` at repo root — declare formatters:
  - `nixfmt` for `.nix`, `prettier` for `.yaml`/`.yml`/`.md`/`.json`, `taplo` for `.toml`, `shfmt` for `.sh`/`.bash`, `ruff format` for `.py`, `tofu` for `.tf`/`.tfvars` (NOT `.hcl`).
...
- [x] 2.1 Add `treefmt`, `prettier`, `taplo`, `shfmt`, `ruff`, and `opentofu` (if not already present) to `flake.nix` devShell `packages` (in the `mkDevShell` `with pkgs;` block). Keep existing `nixfmt` — it serves both `nix fmt` and `treefmt` dispatch.
...
- [x] 4.1 Add a formatting step to `.github/workflows/ci.yml` in the `validate` job — runs `nix develop --command treefmt --fail-on-change` (or equivalent devShell-entering invocation). Confirm it fails with non-zero exit when unformatted files exist and prints paths.
...
- [x] 9.3 Test pre-commit hook: stage an intentionally unformatted file (`*.nix`, YAML, or `*.py`), run `lefthook run pre-commit`, confirm it formats and re-stages. Then test bypass with `--no-verify` — confirm CI catch.

- [x] 9.4 Update `tasks.md` checkboxes to reflect completion.
