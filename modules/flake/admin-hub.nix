# Admin-hub deployment aspect (dendritic Stage 7, D-053). Published from this
# discovered contributor and selected only on `la-admin-1` (S7-2). It owns the
# coupled admin remainder behind one placement statement: the public
# `applications.admin` option namespace, shared `/srv/data` ACL/reconcile and
# admin SSH identity wiring, and the Termix, Vaultwarden, Homepage, Gatus,
# Beszel hub, Webhook, and current Quantum policy composition. The Kanidm
# server composition lives in the co-selected `identity-provider` concern and
# the Cockpit composition in the independently placed `cockpit` concern.
#
# Policy co-selection (D-050/D-053): this concern is only placeable together
# with `identity-provider` (which owns the `services.admin.kanidm.*` option
# namespace this file configures) and `identity-client` (which owns
# `services.identity.oidc.clients.*`, read by the Termix/Quantum/Vaultwarden
# composition below). `la-admin-1` selects all three aspects; a lone selection
# fails evaluation loudly with the missing sibling option namespace. There are
# no cross-aspect imports — the coupling is config reads only.
{ ... }:
{
  flake.modules.nixos.admin-hub =
    ...
    in
    {
      ...
      config = lib.mkMerge [
        # Selecting this aspect is the capability's top-level enablement. It is
        # declared outside the enable gate below, which reads it.
            ...
            # Base config with assertions and default service enables
            {
              services = {
                admin = {
                  ...
                  # `kanidm.enable` is owned by the `identity-provider` concern,
                  # which supplies it as that aspect's top-level enablement (D-053).
