...
- active provider boundary is the discovered `oci` aspect (`modules/flake/oci.nix` + private `modules/flake/_oci/default.nix`); the `modules/providers/` root was deleted in Stage 7 (D-053)
- active storage boundary is host-local: `modules/hosts/oci-melb-1/disko-single-disk-split.nix` (OCI recovered shape) and `modules/hosts/home-forge/disko-two-disk.nix`; there is no shared `modules/storage/` menu
- active reusable module boundaries are the concern-owned aspect contributors under `modules/flake/`: foundation `base`/`shell`/`networking`/`tailscale`/`notify`; operational `backups`/`builder-access`/`observability-agent` (the combined backups aspect was later split by D-055); application and placement `dj`/`music`/`identity-client`/`oci`/`edge`/`cockpit`/`push-server`/`identity-provider`/`admin-hub`/`paperless`/`postgres`/`ai-gateway`/`karakeep`/`niks3-cache`/`phoenix`/`omniroute`; and the infrastructure support quartet `provenance`/`oci-images`/`fleet-packages`/`web-policy`. Private implementations live under the concern-owned `modules/flake/_aspects/`, `_backups/`, `_builder-access/`, `_dj/`, `_edge/`, and `_oci/` paths or under the transitional `modules/services/` boundary; the former `modules/shared/`, `modules/applications/`, and `modules/providers/` roots are gone
