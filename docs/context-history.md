...
- the `make-oidc-contract-intrinsic` change (D-059) retired the Stage 5 `identity-client` bundle: the OIDC contract (`services.identity.oidc.*` — provider URL, client path prefix, token URL, and the five endpoints of each enabled client, derived from `policy/identity.json` and the resolved `kanidm-admin` web-policy route) is now the intrinsic module `modules/identity/_oidc.nix`, imported by every reader instead of selected — the provider leaf `kanidm-runtime.nix`, the capability `kanidm-host-auth.nix`, and the consuming leaves `paperless/core.nix`, `karakeep.nix`, and `termix.nix`
