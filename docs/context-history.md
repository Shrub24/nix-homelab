...
- active reusable module boundaries are the foundation aspects (`modules/flake/aspects.nix` + `modules/flake/_aspects/`), the operational aspects (`backups`, `builder-access`, `observability-agent` in `modules/flake/aspects.nix`), the `tailscale` service leaf (`modules/services/tailscale.nix`), and the remaining shared leaves (`modules/shared/`)
