...
- [x] 2.2 Create `modules/applications/dj/engine-dj.nix`: library share (`/srv/data/engine-dj/library`, rw) and writable media share wiring against the VM instance
  - refs: `specs/engine-dj-library/spec.md` (stable mapping, direct media import)
...
- [x] 4.2 Execute validation pass: guest boot, Engine launch on shared library, track import, SC6000 Remote Library connect over bridge, DB integrity across guest reboot
  - refs: `specs/engine-dj-library/spec.md` (validated shared-library operation)
  - notes: validated live across track import, SC6000 Remote Library, guest/host reboots, and SQLite integrity checks; installer staging now no-ops when source and destination are identical
- [x] 4.3 Resolve the spike outcome: retain the shared layout on success, or switch to guest-local qcow2 DB + copy-in/copy-out on failure
  - refs: design R1 fallback
  - notes: not triggered; the split M:/L: virtiofs layout passed, and copy-in/copy-out remains the regression fallback
...
- [x] 5.2 Confirm writable media import from a guest session
  - notes: Engine recurring import requires M: write access; direct import, analysis, and playback passed
- [x] 5.3 Confirm conflict semantics: starting worker stops VM and vice versa; backup never overlaps a writer
  - notes: validated through Linux-side Engine DB publication and quiesced restic runs; both completed with the VM stopped and restored it afterward
