...
- Add an Engine DJ application module as the first consumer: a dedicated virtiofs share exposing `/srv/data/engine-dj/library` (ext4) into the guest at a stable drive letter (`L:`), with a fixed guest-side junction chain (`M:\Engine Library` → `L:\`, then `Music\Engine Library` → `M:\Engine Library`) because Engine's path model does not tolerate relocation.
...
- Keep the Engine SQLite database (`Database2/m.db`) on the shared ext4 directory so Linux workers can operate on it directly when the VM is down; music files remain ordinary Linux files under the host music root `/srv/storage/media/music`, shared into the guest.
...
- **Validated risks:** Engine's SQLite database and recurring track imports work over the split virtiofs layout, including guest/host reboots and SC6000 Remote Library. Engine DJ's Qt6/OpenGL requirements make virtio-gpu driver installation mandatory in the guest; Remote Library discovery requires same-L2 presence, which constrains network mode choices.
