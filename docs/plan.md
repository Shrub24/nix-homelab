...
- `TD-26` (`partially resolved`) — consuming the shared nix-fleet aspects: nix-fleet publishes `beszel-agent`, `builder-access`, `niks3-cache`, `niks3-publisher`, `notification-daemon` and `tailscale`. Four are consumed through convention contributors (`modules/flake/tailscale.nix`, `modules/flake/observability-agent.nix` → `beszel-agent`, `modules/flake/builder-access.nix`, `modules/cache/niks3-cache.nix` → the shared `services.niks3-cache.s3.*` surface), verified per host on structured observables (option values, `sops.secrets`, systemd unit wiring) with only the intended deltas: the `beszel-agent` namespace rename, the newly bound `services.builder-access.hosts`, upstream's `ControlPath` socket name, and `secretFiles.apiToken` replacing `hostSecretFile`. Remaining: `cache-publisher` (our activation-time post-deploy push with the public-key filter versus nix-fleet's per-build hook — deletion is decided by whether CI-built closures via nixbuild.net must stay published) and `notification-daemon` (our `notify` carries the daemon body plus monitoring composition, so its mechanism half needs upstreaming before the aspect can be consumed).
