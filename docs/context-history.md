...
- active reusable module boundaries are the concern-owned aspect contributors under `modules/flake/` (foundation `base`/`shell`/`networking`/`tailscale`/`notify`, operational `backups`/`builder-access`/`observability-agent`, application `dj`/`identity-client`) plus the infrastructure support quartet (`provenance`, `oci-images`, `fleet-packages`, `web-policy`), the concern-owned private paths `modules/flake/_aspects/`, `_backups/`, `_builder-access/`, and the `tailscale` service leaf (`modules/services/tailscale.nix`); the former `modules/shared/` catch-all is gone
