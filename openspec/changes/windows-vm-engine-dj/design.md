...
libdjinterop workers need native filesystem access from Linux. An NTFS image would make the DB unreadable without mounting foreign filesystems; a guest-local qcow2 hides the DB from Linux entirely. virtiofs presents an ext4 directory as a guest drive letter (WinFsp-backed, behaves closer to local disk than SMB). The live validation pass confirmed this layout works across imports, Engine restarts, and guest/host reboots.

**D3 — Fixed symlink inside the guest.**
Engine requires path-stable libraries; mapped drives alone fail upstream. The guest gets a permanent drive letter for the library share (`L:`) and a one-time `mklink /D` junction chain (`M:\Engine Library` → `L:\`, then `Music\Engine Library` → `M:\Engine Library`). Documented in the guest setup runbook, not automated by Nix.
...
**D5 — Music shared read-write from the host music root `/srv/storage/media/music`; library share scoped separately to `/srv/data/engine-dj/library`.**
Two separate shares: media (`M:`, whole music root) and library (`L:`, the NVMe Engine database) with a guest junction `M:\Engine Library` → `L:\` — no host bind inside the music root. Engine requires write-capable media for recurring imports; replication policy and other media-tree protections remain owned by the music application.
...
- **[R1] SQLite-over-virtiofs locking regression** → Live validation passed across import, SC6000 use, and reboots. If a future virtiofs/QEMU change breaks this contract, fall back to a guest-local qcow2 DB with copy-in/copy-out during VM-stopped windows.
...
2. Operator creates the VM (install media out-of-band) and runs the validation pass per spec.
3. The validation pass succeeded; the split M:/L: layout is settled and the guest-local database remains a regression fallback.
