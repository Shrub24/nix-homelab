...
- **PL-3:** Store SQLite state and inbound M3Us under `/srv/data/traktor-m3u-sync`, outside the media library; the engine export writes playlists directly into the Engine library database at `<engine-library>/Database2/m.db`, so they never surface as audio in Navidrome or Syncthing sync scope.
