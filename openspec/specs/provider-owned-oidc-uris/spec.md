...
## Purpose
Define one owner for OIDC endpoint URIs: the canonical OIDC contract (`modules/identity/_oidc.nix`, intrinsic — imported by every participant that reads it rather than selected as a placement capability) derives them from the provider public URL and the enabled client registry, emits them read-only, and consumers reference those outputs instead of reconstructing URIs themselves.
