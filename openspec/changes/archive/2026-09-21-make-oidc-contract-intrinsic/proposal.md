...
4. **The canonical single-derivation helper is dead code and models the wrong provider.** `lib/policy.nix:24` defines `mkOidcEndpoints` and `openspec/specs/provider-owned-oidc-uris/spec.md` requires it to be the one derivation with "identical logic across all call sites"; it has **zero call sites** in `lib/`, `modules/`, `tests/`, `scripts/`, or `opentofu/`. Its shape is Pocket ID's (`<issuer>/authorize`, `<issuer>/api/oidc/token`, `<issuer>/api/oidc/userinfo`), which differs from Kanidm's in three of five fields; Kanidm's authorization and token endpoints are provider-level, so a single-issuer-base signature cannot express them.
...
6. **One derivation site.** The dead Pocket-ID-era `mkOidcEndpoints` helper is deleted from `lib/policy.nix` and the stale requirement that mandates it is removed; the contract file stays the single derivation site that consumers read.
