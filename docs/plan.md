...
- `TD-31` (`resolved by nix-fleet publishing the feature as a module function`) — the interim consumption of nix-fleet's fleet feature (D-066) is gone. `flakeModules.fleet` now imports its own inventory, so a consumer receives `config.fleet.*` and `config.fleet.realization` bound to *its* evaluation: the contributor collapsed to `imports = [ inputs.nix-fleet.flakeModules.fleet ]` plus `imports = [ config.fleet.realization ]`, the two path imports disappeared, and the scaffold ratchet asserts that documented shape. The feature also contributes `checks.fleet-render` and the CI builder bundles (`packages.ci`, `packages.fixture`) in this repository's evaluation, built green; adopting those bundles as the CI build path is `TD-30`.
