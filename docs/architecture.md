...
- `modules/music/_dj/` for the DJ composition root (Engine DJ library hosting on a Windows VM; see `docs/runbooks/engine-dj-guest-setup.md`), imported only by the discovered `dj` aspect
- concern-owned contributors for the published `flake.modules.nixos.<aspect>` records, located in their domain directory since Stage 8 (D-056) — foundation `modules/flake/{base,shell,networking,tailscale}.nix`; notifications `modules/notifications/{notify,push-server}.nix`; operational `modules/cache/{state-backups,cache-publisher}.nix` and `modules/flake/{builder-access,observability-agent}.nix`; application/identity `modules/music/{music,dj}.nix` and the two-contributor `identity-client` (`modules/identity/identity-oidc.nix`, `modules/identity/kanidm-host-auth.nix`); the fleet contract `modules/fleet/internal-contracts.nix`; placement `modules/oci/oci.nix`, `modules/edge/edge.nix`, `modules/admin/{cockpit,termix,vaultwarden,homepage,gatus,beszel,webhook}.nix`, `modules/identity/identity-provider.nix`, `modules/cache/niks3-cache.nix`, and `modules/flake/{paperless,postgres,ai-gateway,karakeep,phoenix,omniroute}.nix` (each owning its capability's top-level enablement; `admin-hub` was removed and its members split into individual aspects by D-054) — plus the infrastructure support quartet `provenance`, `oci-images`, `fleet-packages`, `web-policy` in `modules/flake/`, with private implementations beside their owner under `modules/flake/_aspects/`, `modules/flake/_builder-access/`, `modules/cache/_backups/`, `modules/music/_dj/`, `modules/edge/_edge/`, and `modules/oci/_oci/`. The Stage 6 music leaves remain under the transitional `services` root (`modules/services/music/**`) with a recorded retirement criterion: when `modules/services/` converts they either move beside the music contributor under `modules/flake/_music/**` or become top-level contributors if they gain independent placement
...
- `modules/oci/_oci/default.nix` for OCI-specific host-safe defaults, imported only by the discovered `oci` aspect (`modules/oci/oci.nix`, selected on `oci-melb-1`)
...
- the operational capability is split (D-055): `state-backups` composes `modules/services/state-backups.nix` and derives the conventional host secret path `secrets/hosts/<host>/system.yaml` plus the `shrublab-backup-<host>` bucket, gating enablement on the secret file's existence (two-step sops bootstrap); `cache-publisher` owns the upstream `niks3-auto-upload` module and the leaves `modules/cache/_backups/niks3-upload-client.nix` and `modules/cache/_backups/niks3-post-deploy.nix`, resolves its write endpoint through the `niks3Write` internal contract (D-056), injects the post-deploy `nix-path-filter` package per system, and asserts `services.notification-daemon.monitor.enable` (the `notify` aspect owns monitor composition) without importing `notify`. The classified `nix.settings.post-build-hook = lib.mkForce ""` suppression is required because upstream `niks3-auto-upload` has no separate hook-disable option. The OCI cache server (`modules/services/niks3.nix`) remains a leaf.
...
- Only hosts push, post-deploy, via `modules/cache/_backups/niks3-post-deploy.nix` (composed by the `cache-publisher` aspect; the activation-triggered `niks3-hook send` reuses the upstream `niks3-auto-upload` daemon/socket while the automatic Nix post-build-hook stays suppressed by the classified `nix.settings.post-build-hook = lib.mkForce ""`)
...
Phase-1 ingress is implemented with the discovered `edge` aspect (`modules/edge/edge.nix` + private `modules/edge/_edge/edge-ingress.nix`) over `modules/services/edge-proxy-ingress.nix`.
