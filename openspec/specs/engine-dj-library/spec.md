# engine-dj-library Specification
...
### Requirement: Music is imported from the authoritative host tree

Music files exposed to the guest SHALL come directly from the authoritative host media tree. The share SHALL be writable because recurring Engine imports require source-drive writes.

#### Scenario: Guest imports directly from media
- **WHEN** Engine imports tracks from the mounted music share
- **THEN** the share permits the access pattern Engine requires
...
### Requirement: Validated shared-library operation

The settled layout SHALL preserve Engine DJ operation across guest boot, track import, SC6000 Remote Library connection over the bridge, and database integrity across guest and host reboots.

#### Scenario: Future regression triggers fallback
- **WHEN** a future virtiofs or QEMU change causes database corruption or unusable locking on the shared layout
- **THEN** operators fall back to a guest-local database with copy-in/copy-out exchange during VM-stopped windows, preserving Linux-side tooling
