...
- music service modules regrouped under `modules/services/music/` as a coherent feature subtree, with the Stage 6 private implementation leaves for Beets secrets/CLIs, ingest, and storage/permission ownership (D-052)
- the music stack is composed by the discovered home-forge-only `music` deployment aspect (`modules/flake/music.nix`, selected via `modules/flake/registry.nix`), whose selection provides `applications.music.enable`; `dj` remains a separately selected aspect and consumes the read-only `applications.music.contract.{storageRoot,libraryDir,playlistsDir}` — worker adoption of `libraryDir`/`playlistsDir` in the Engine export job paths is deferred (D-052)
...
- discovered concern contributors `modules/flake/<concern>.nix` for feature composition roots (multi-service stacks); Stage 6 converted music (`modules/flake/music.nix`) and Stage 7 converted admin/edge/DJ/provider/product placement (D-052, D-053), so no `modules/applications/` root remains
- `modules/services/<domain>/<name>.nix` for reusable service modules grouped by domain (e.g. `modules/services/music/navidrome.nix`, `modules/services/music/audiomuse.nix`, `modules/services/music/syncthing.nix`, `modules/services/music/ingest.nix`, `modules/services/music/storage.nix`, and the Beets owner at `modules/services/music/beets/{default.nix,runners.nix,files/}`)
...
- `modules/flake/_dj/` for the DJ composition root (Engine DJ library hosting on a Windows VM; see `docs/runbooks/engine-dj-guest-setup.md`), imported only by the discovered `dj` aspect
- concern-owned `modules/flake/*.nix` contributors for the published `flake.modules.nixos.<aspect>` records — the five foundation aspects `base`, `shell`, `networking`, `tailscale`, `notify`, the three operational aspects `backups`, `builder-access`, `observability-agent`, and the `dj`, `music`, and `identity-client` deployment aspects (`music` is home-forge only and imports its private leaves; `identity-client` has two contributors), the thirteen Stage 7 placement aspects `oci`, `edge`, `cockpit`, `push-server`, `identity-provider`, `admin-hub`, `paperless`, `postgres`, `ai-gateway`, `karakeep`, `niks3-cache`, `phoenix`, `omniroute` (each owning its capability's top-level enablement) — plus the infrastructure support quartet `provenance`, `oci-images`, `fleet-packages`, `web-policy`, with private implementations under the concern-owned paths `modules/flake/_aspects/`, `_backups/`, `_builder-access/`, `_dj/`, `_edge/`, and `_oci/`. The Stage 6 music leaves remain under the transitional `services` root (`modules/services/music/**`) with a recorded retirement criterion: when `modules/services/` converts they either move beside the music contributor under `modules/flake/_music/**` or become top-level contributors if they gain independent placement
...
- `modules/flake/_oci/default.nix` for OCI-specific host-safe defaults, imported only by the discovered `oci` aspect (selected on `oci-melb-1`)
- host-local disko layouts beside their consumer: `modules/hosts/oci-melb-1/disko-single-disk-split.nix` (split root/data/nix/media), `modules/hosts/home-forge/disko-two-disk.nix`; the shared `modules/storage/` menu was removed in Stage 5
...
- `modules/hosts/oci-melb-1/disko-single-disk-split.nix` is the canonical declarative boundary for that recovered host shape
- host-specific sizing stays in `modules/hosts/oci-melb-1/default.nix`, while the partition/mount contract remains declarative in the host-local layout
...
- `applications.music` requires explicit `storageRoot`/`dataRoot` host bindings with no fleet defaults (`policy/globals.nix` carries no music paths); the discovered `modules/flake/music.nix` aspect (selected only on `home-forge`) owns composition and injects the derived paths into the private `modules/services/music/{beets,ingest,storage}` leaves, with the storage leaf owning creation of the shared roots and layout directories under `storageRoot`; lower-level service modules may add ACLs, marker files, or service-specific subdirectories, but do not redefine those shared root directory ownership contracts
...
Fleet hosts own physical networking through the import-activated `networking` foundation aspect (published from `modules/flake/networking.nix`; private leaf `modules/flake/_aspects/networking.nix`), not scripted networking or dhcpcd:
...
- host recovery secret registration remains feature-owned by `modules/flake/_aspects/host-recovery.nix`, while hosts only bind the host secret file path and enable the feature
...
- `la-admin-1` hosts the private admin surface composition under the discovered `admin-hub`/`identity-provider`/`cockpit` concerns (`modules/flake/{admin-hub,identity-provider,cockpit}.nix`)
...
Phase-1 ingress is implemented with the discovered `edge` aspect (`modules/flake/edge.nix` + private `modules/flake/_edge/edge-ingress.nix`) over `modules/services/edge-proxy-ingress.nix`.
