...
- Key files: `modules/flake/registry.nix`, `modules/flake/base.nix`, `modules/flake/dj.nix`, `modules/flake/music.nix`, `modules/flake/_aspects/base.nix`, `modules/flake/_unconverted-nixos-dirs.nix`
...
- Contains: Application modules with `enable` flags, sub-service wiring, shared paths, ACLs. The music stack was converted in dendritic Stage 6 (D-052) into the discovered home-forge-only `music` aspect `modules/flake/music.nix`; its private implementation leaves live under `modules/services/music/**`.
- Key files: `modules/applications/admin/default.nix`, `modules/applications/edge-ingress.nix`, `modules/flake/music.nix`
