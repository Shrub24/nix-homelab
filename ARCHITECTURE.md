...
2. `modules/web/web-policy.nix` validates the policy before resolution — unknown host keys and host-backed origins whose FQDN does not match a declared record's derived `tailscale.fqdn` fail closed with named `web-policy:` errors
...
- Pattern: every service in the policy is either published or private. A published service exposes its public identity and the published upstream *shape* (scheme + port) in `repo.web.catalog.<id>` and never its origin host; a private service (declared `tailscale-only` + `declarePublic = false`) exposes `publicUrl`/`publicHost = null` and an `endpoint { scheme, host, port, url }` instead. The declaration is the single source for both sides: the provider's listen port and every consumer's target URL derive from the same entry, so the port cannot drift. The edge stays the one consumer of the full `repo.web.hosts` resolution, which carries origins. A route whose provider socket is not directly reachable declares `exposureMode = "tailscale-serve"`; the provider host receives that route through `repo.web.originServices`, renders its own front on the declared port, and the edge dials the provider's canonical private name with public-root verification (D-063).
