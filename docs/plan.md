...
- `TD-22` (`spec-hygiene`) — 20 canonical capabilities in `openspec/specs/**` still carry the placeholder Purpose the archive tool writes (`TBD - created by archiving change …, Update Purpose after archive.`). `openspec validate --all --strict` reports them as failures, which makes the repo-wide strict gate unusable as a signal; per-change strict validation is unaffected and remains part of every change's gates. Fix is one real sentence per capability, derived from that capability's own requirements.
