...
- Only hosts push, post-deploy, via `modules/flake/_backups/niks3-post-deploy.nix` (composed by the `cache-publisher` aspect; the activation-triggered `niks3-hook send` reuses the upstream `niks3-auto-upload` daemon/socket while the automatic Nix post-build-hook stays suppressed by the classified `nix.settings.post-build-hook = lib.mkForce ""`)
...
- admin workloads consume the canonical identity-client OIDC contract and never configure the provider; shared endpoint data comes from web policy
- Quantum is removed from the active module graph and disabled/deferred: its former module and host files (`modules/services/admin/quantum.nix`, `modules/hosts/la-admin-1/quantum.nix`) were deleted, and the residual `/srv/data` operator ACL/reconcile unit and Quantum SSH secret registrations live as explicit `la-admin-1` host-local configuration (`modules/hosts/la-admin-1/_admin-runtime.nix`). Any future re-enable lands as a self-contained aspect consuming canonical identity contracts and does not restore admin-hub coupling
