...
- **Provider isolation:** Cloud/platform quirks live in the host-selected `oci` aspect (`modules/oci/oci.nix` + private leaf `modules/oci/_oci/default.nix`) — workload modules stay provider-agnostic; the legacy `modules/providers/` root was deleted in Stage 7 (D-053)
...
- Contains: Input pins (`nixpkgs`, `disko`, `sops-nix`, `deploy-rs`, `niks3`, `flake-parts`, `import-tree`) and `denful/import-tree` discovery of `modules/` with one enumerated `filterNot` boundary (`modules/flake/_unconverted-nixos-dirs.nix`) for the single directory still holding plain NixOS leaves — `services` (`core`/`profiles` left in Stage 2, `shared`/`storage` in Stage 5, `applications`/`providers` in Stage 7 D-053, `hosts` in Stage 8 after every host became a discovered contributor). The filter is transitional and must shrink as converted roots empty; underscore-renaming whole roots is not completion
...
- Depends on: Published aspects (`flake.modules.nixos.<aspect>`) selected by the record, input-provided NixOS modules (`extraModules`), and concern-owned feature sources (aspects, private implementation leaves, transitional `services` leaves)
...
- Used by: The concern-owned aspect contributor that imports them (`modules/<domain>/<concern>.nix` or a private `modules/<domain>/_<concern>/` leaf); host records never import a workload service directly
...
- Location: the discovered `oci` aspect (`modules/oci/oci.nix`) with its private implementation leaf (`modules/oci/_oci/default.nix`)
...
- Location: `modules/services/notification-daemon/default.nix` (imported by the `notify` aspect at `modules/notifications/notify.nix`), `pkgs/notification-daemon/`, `pkgs/notify/`
...
4. the discovered `edge` aspect (`modules/edge/edge.nix` + private `modules/edge/_edge/edge-ingress.nix`) enables the Caddy reverse proxy with role-based configuration (edge/origin/none) — selected by `la-admin-1` (edge) and `oci-melb-1` (origin), with routes projected from this policy for the edge role only
...
4. Both services connect through the shared PostgreSQL mechanism (`modules/services/postgres.nix`, D-058) with niks3-managed backup
...
- Location: `modules/<domain>/<concern>.nix`, publishing `flake.modules.nixos.<aspect>`, with private implementation beside the owner under `modules/<domain>/_<concern>/` (or under the transitional `modules/services/` root)
...
- Location: `modules/edge/edge.nix` (discovered aspect) + private `modules/edge/_edge/edge-ingress.nix`
