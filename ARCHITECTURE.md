...
- Location: concern-owned contributors discovered under `modules/flake/` (foundation `base.nix`, `shell.nix`, `networking.nix`, `tailscale.nix`, `notify.nix`; operational `state-backups.nix`, `cache-publisher.nix`, `builder-access.nix`, `observability-agent.nix`; `dj.nix`, `music.nix`, and the two-contributor `identity-client` (`identity-oidc.nix` + `kanidm-host-auth.nix`); placement `oci.nix`, `edge.nix`, `cockpit.nix`, `push-server.nix`, `identity-provider.nix`, `termix.nix`, `vaultwarden.nix`, `homepage.nix`, `gatus.nix`, `beszel.nix`, `webhook.nix`, `paperless.nix`, `postgres.nix`, `ai-gateway.nix`, `karakeep.nix`, `niks3-cache.nix`, `phoenix.nix`, `omniroute.nix`); private implementation leaves live under the concern-owned paths `modules/flake/_aspects/`, `_backups/`, `_builder-access/`, `_dj/`, `_edge/`, and `_oci/`
