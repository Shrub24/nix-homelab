...
## Stage 4 realignment (2026-09-11)

Stage 4 (`dendritic-stage-4-source-model-realignment`, D-050) realigned the documented source/aspect model without changing deployed behavior. The central `modules/flake/aspects.nix` publication file is replaced by concern-owned discovered contributors; `provenance`, `oci-images`, and `fleet-packages` are classified as infrastructure support modules, not deployment capabilities; `dj` selection enables `applications.dj`; and aspect relationships follow three composition modes (intrinsic composition, policy co-selection, optional integration) instead of a universal no-aspect-import rule. The "80% plain leaves" endpoint framing in the Verdict below is **superseded**: conventional plain class-oriented leaves and the filter are transitional, not the endpoint — the filter must shrink as converted roots empty, and underscore-renaming whole roots solely to hide unchanged code is not completion. Genuine private implementation/data may remain private with underscore-prefixed or otherwise explicit private paths. Web, identity, and music conversions remained deferred at this stage.

## Stage 5 conversion (2026-09-12)

Stage 5 (`dendritic-stage-5-shared-source-contributors`, D-051) removed the last two legacy evaluator-class roots. `modules/storage/` was deleted (both templates had zero consumers; disko layouts are host-local) and `modules/shared/` was emptied and removed with no default or wrapper. The four private implementation leaves moved beside their aspect owners under the concern-owned underscore paths — `_aspects/host-recovery.nix` (base), `_backups/niks3-{upload-client,post-deploy}.nix` (backups), `_builder-access/nixbuild-ssh.nix` (builder-access) — and `web-policy` became the discovered all-host support contributor `modules/flake/web-policy.nix`. `identity-client` is the first production instance of D-050's multi-contributor single-aspect merge: `identity-oidc.nix` and `kanidm-host-auth.nix` are separately discovered top-level contributors that each nest their own options/config inline in `flake.modules.nixos.identity-client`, with no `_identity-client/` private leaves and no wrapper, selected on OCI/LA only. The one temporary filter shrank from six entries to exactly `applications`, `hosts`, `providers`, `services`. The conversion supersedes only D-050's deferral of web-policy and identity-client; music remained deferred at this stage and was converted in Stage 6 (below).

## Stage 6 conversion (2026-09-13)

Stage 6 (`dendritic-stage-6-music-composition`, D-052) converted the application that D-050/D-051 had left deferred. The directly host-imported evaluator-class coordinator `modules/applications/music/default.nix` (and its `files/` directory) was deleted; the music stack is now published as the discovered, home-forge-only `music` deployment aspect `modules/flake/music.nix`, whose selection provides `applications.music.enable`, with `modules/flake/registry.nix` selecting `aspects.music` on `home-forge` only. Composition ownership (public options plus the read-only `applications.music.contract`, shared path derivation, feature choices, service/backup wiring, secret-file passthrough, runner instances, success-chain intent) stays in the aspect; concrete implementation ownership moved to private leaves under `modules/services/music/**` — the Beets owner (`beets/default.nix`: SOPS secrets/templates, `services.beets.renderedConfigFiles`, the four operator CLIs, moved config assets under `beets/files/`), the new ingest leaf (`ingest.nix` + `files/ffmpeg-preprocess.sh`), and the new storage leaf (`storage.nix`: groups, media tmpfiles/ACLs, permission reconcile, `media-fixperms`). `beets/runners.nix` is unchanged. `dj` remains a separately selected aspect and consumes `applications.music.contract.{storageRoot,libraryDir,playlistsDir}` through `config.applications.music.contract or null` with a named assertion; worker adoption of `contract.libraryDir`/`contract.playlistsDir` in the Engine export paths is deferred. The four-root filter remains exactly `applications`, `hosts`, `providers`, `services`; the recorded retirement criterion is that when `modules/services/` is scheduled for conversion these leaves either move beside the music contributor under a concern-owned underscore path (`modules/flake/_music/**`) or become top-level contributors if they gain independent placement. There is no secret bootstrap change (no `secrets/**`, `.sops.yaml`, key, or ciphertext edit), and the change is implementation-complete but not deployed and not archived.
...
The repo should move to **plain flake-parts + import-tree + named `flake.modules` aspects**, keeping leaf NixOS modules almost entirely unchanged behind a new composition surface *(scoping-time framing, superseded by the Stage 4 realignment — see note above)*. Den is deferred. The refactor is real — today the flake is hand-rolled `nixosSystem` with broad `specialArgs` — but roughly 80% of `modules/services/**` survives verbatim *(transitional baseline, not the endpoint)*; what actually changes is `flake.nix`, `hosts/`, `modules/applications/`, `modules/profiles/`, and the secret-registration contracts.
