...
- `TD-05` (`deferred`) — `backups` conflates restic recovery with cache publication. Owner: `openspec/changes/split-state-backups-cache-publication/`. Overlaps with the Stage 8 Niks3-write contract, so ordering must be explicit and implementations must not run concurrently; Stage 8 consumes the final owner if the split lands first.
