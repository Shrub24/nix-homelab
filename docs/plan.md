...
- `modules/flake/` concern-owned aspect contributors (`base.nix`, `shell.nix`, `networking.nix`, `tailscale.nix`, `notify.nix`, `backups.nix`, `builder-access.nix`, `observability-agent.nix`, `dj.nix`, `music.nix`, `identity-oidc.nix`, `kanidm-host-auth.nix`) plus infrastructure support modules (`provenance.nix`, `oci-images.nix`, `fleet-packages.nix`, `web-policy.nix`) and the concern-owned private paths `modules/flake/_aspects/`, `_backups/`, `_builder-access/`
