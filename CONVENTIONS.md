...
- Plain class-oriented NixOS leaves and the four-directory import-tree filter (`applications`, `hosts`, `providers`, `services`) are transitional, not the endpoint: the filter must shrink as converted roots empty, and underscore-renaming whole roots solely to hide unchanged code is not completion. Genuine private implementation/data may remain private with underscore-prefixed or otherwise explicit private paths.
