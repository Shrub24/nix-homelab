...
- Several discovered top-level contributors may define the same `flake.modules.nixos.<aspect>` when a capability is composed from independent source files: `cache-publisher` is the current example (`modules/cache/cache-publisher.nix` plus its `upload-client` and `post-deploy` siblings, each nesting its own options/config inline). Do not add a private leaf directory for the merge, a central wrapper, or a cross-contributor import for this pattern.
