...
- `TD-06` (`resolved by normalize-notification-policy`) — the ntfy publisher policy is a typed map keyed by canonical host ID in `modules/notifications/push-server.nix`, passed into the aspect by value; an unknown ID fails the aspect's assertion and the module renders the publishers into `auth-access` while the bcrypt hashes stay in the encrypted auth file. LA's literal list is gone.
...
- `TD-09` (`follow-up`) — Cockpit resolves its route from the hardcoded canonical key `"cockpit-admin"` (`modules/admin/cockpit.nix`, `modules/admin/cockpit/loopback-tls.nix`, `modules/admin/homepage/_data.nix`), while the host that selects it, `oci-melb-1`, forces `publicHost`/`urlRoot` away from the leaf defaults because it publishes behind the LA edge (`modules/hosts/oci-melb-1/_cockpit-auth.nix`). Follow-up: a typed/explicit route-key contract.
...
- `TD-26` (`partially resolved`) — consuming the shared nix-fleet aspects: nix-fleet publishes `beszel-agent`, `builder-access`, `niks3-cache`, `niks3-publisher`, `notification-daemon` and `tailscale`. Four are consumed through convention contributors (`modules/flake/tailscale.nix`, `modules/flake/observability-agent.nix` → `beszel-agent`, `modules/flake/builder-access.nix`, `modules/cache/niks3-cache.nix` → the shared `services.niks3-cache.s3.*` surface), verified per host on structured observables (option values, `sops.secrets`, systemd unit wiring) with only the intended deltas: the `beszel-agent` namespace rename, the newly bound `services.builder-access.hosts`, upstream's `ControlPath` socket name, and `secretFiles.apiToken` replacing `hostSecretFile`. `cache-publisher` also consumes its shared mechanism now (the activation-time post-deploy push with the public-key filter is retired with `pkgs/nix-path-filter`; closures built on nixbuild.net are no longer published by anything, which the fleet accepted). Remaining: `notification-daemon` (our `notify` carries the daemon body plus monitoring composition, so its mechanism half needs upstreaming before the aspect can be consumed).
